...
data.frame(mPEHE_T = mPEHE_T, mPEHE_S = mPEHE_S, mPEHE_nvX = mPEHE_nvX, mPEHE_M = mPEHE_M,
...
# Naive X_learner : 
nvX_hat = nvX_Learner(as.data.frame(X), Y, W, r_hat, T.Fit, model = "randomForest")
...
# M_learner
M.Fit = M_Learner(X, Y, W, r_hat, model = "lm", p = 2)
